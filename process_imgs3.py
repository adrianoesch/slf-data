...
def findSwissContourImg(img):
    ...
    while True:
        ...
        if newWidth < template.shape[1] or (len(maxVals)>10 and maxVals[-1]<maxVals[-2]):
            break
        else:
            i=i+1
    ...
    maxVal = max(maxVals)
    maxIter = maxVals.index(maxVal)
    newImg = imutils.resize(img,width=img.shape[1]-maxIter*jump)
    swiss = newImg[maxLocs[maxIter][1]:maxLocs[maxIter][1]+template.shape[0], maxLocs[maxIter][0]:maxLocs[maxIter][0]+template.shape[1],:]
    return swiss
...
def getOverlayMap(d,alpha=0.6,draw=False):
    t = np.ones(template.shape)*255
    for x,coordStr,v in d:
        if v is 'NA':
            continue
        coords = coordDic[coordStr]
        cv2.rectangle(t,(coords[0]+1,coords[2]+1),(coords[1]-1,coords[3]-1),color=cols[v],thickness=-1)
        # cv2.rectangle(t,(y1+1,x1+1),(y2-1,x2-1),color=cols[v],thickness=-1)
    o = templateGrid.copy()
...
def findCat(img,v,hists):
    ...
    if nWhite > img.shape[0]*img.shape[1]*0.5:
        return 'NA'
